...
const { chooseFilter } = require("./Filters");
...
async function applyAudioToVideoFILE(interaction, file, start, end, danceEnd) {
    ...
    const reducerNum = Math.max(Math.round((modifier == 2 ? 25 : maxMB / (duration - duration * sizeModifier)) * 10) / 10, 0.1); //2x2
    ...
    let modifiers = {
        ...
        vbit: 0,
        abit: 0,
        vfilter: `setpts=${beat / bpm}*PTS,scale=iw*${reducer > 1 ? 1 : reducer}:-1`,
    };
...
async function chooseFilter(filter, modifier, normalizedAudioSpeed, volume, beat, bpm) {
    ...
    switch (filter) {
        ...
        case config.Filters.troll:
            ...
            modifier.abit = "1k";
            modifier.vbit = "10k";
            modifier.vfilter = `setpts=${beat / bpm}*PTS,scale=iw:-1`;
